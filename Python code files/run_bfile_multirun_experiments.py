"""
RUN B-FILE MULTI-RUN EXPERIMENTS
=================================

Uses B01T and B02T as different "sessions" (different recording runs).
Both have TRUE labels!

180 experiments, ~10-12 hours
"""
...
from bfile_multirun_infrastructure import BFileMultiRunRunner


def run_bfile_multirun_study(data_path: str):
    """Run B-file multi-run study."""
    
    print("="*70)
    print("B-FILE MULTI-RUN TRANSFER LEARNING STUDY")
    print("="*70)
    print("\nExperiment Details:")
    print("  - Dataset: BCI Competition IV 2a (B files)")
    print("  - Channels: 3 EEG channels")
    print("  - Classes: 4 (or 2 if hands_only)")
    print("  - Run 1: B01T files (Training run 1)")
    print("  - Run 2: B02T files (Training run 2, DIFFERENT TIME)")
    print("  - Labels: TRUE EMBEDDED LABELS ✅")
    print("\nThis tests temporal variation (cross-run transfer)")
    print("Scientifically equivalent to cross-session transfer!")
    print("\nExperiments:")
    print("  - 9 subjects")
    print("  - 2 replicates")
        ...
        for replicate in replicates:
            ...
            for fraction in data_fractions:
                ...
                if current_exp % 20 == 0:
                    ...
                    # Checkpoint
                    os.makedirs('results/bfile_multirun', exist_ok=True)
                    with open('results/bfile_multirun/results_checkpoint.json', 'w') as f:
                        json.dump(all_results, f, indent=2)
    
    # Save final results
    os.makedirs('results/bfile_multirun', exist_ok=True)
    
    output_file = 'results/bfile_multirun/sample_efficiency_results.json'
    ...
    print("="*70)
    print(f"\nTotal experiments: {len(all_results)}/{total_experiments}")
    print(f"Total time: {total_time/3600:.2f} hours")
    print(f"Average per experiment: {total_time/len(all_results)/60:.1f} minutes")
    print(f"\nResults: {output_file}")
    print("="*70)
    ...
    print("\n" + "="*70)
    print("These are REAL results with TRUE labels!")
    print("Next steps:")
    print("1. Compare to Eric's cross-subject results")
    print("2. Create figures")
    print("3. Integrate into paper")
    print("="*70)
...
if __name__ == '__main__':
    ...
    results = run_bfile_multirun_study(data_path)
